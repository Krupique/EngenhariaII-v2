...
public class BuscaComprasController implements Initializable {
    ...
    private void exibir_tableview(ArrayList<Object[]> lista, int flag) {
        ...
        for (int i = 0; i < lista.size(); i++) {
            temp = new Object[5];
            temp[0] = lista.get(i)[0];
            temp[1] = lista.get(i)[1];
            temp[2] = lista.get(i)[2];
            temp[3] = lista.get(i)[3];
            temp[4] = lista.get(i)[4];
            obj_comp = new Objeto(String.valueOf((int)temp[0]), (String)temp[1], 
                    (String)temp[2], String.valueOf((int)temp[3]), String.valueOf((double)temp[4]));
            
            obslist_comp.add(obj_comp);
            
            arry_parc = new ArrayList<>();
            arry_parc = (ArrayList<Object[]>)lista.get(i)[5];
            for (int j = 0; j < arry_parc.size(); j++) {
                if(((int)arry_parc.get(j)[1] == 0 && flag == 0) || flag == 1)
                {
                    temp2 = new Object[5];
                    temp2[0] = arry_parc.get(j)[0];
                    temp2[1] = arry_parc.get(j)[1];
                    temp2[2] = arry_parc.get(j)[2];
                    temp2[3] = arry_parc.get(j)[3];
                    temp2[4] = arry_parc.get(j)[4];

                    obj_parc = new Objeto(String.valueOf((int)temp2[0]), String.valueOf((int)temp2[1]), 
