...
public class CadClienteController implements Initializable {
    ...
    private void evtBuscar(ActionEvent event) {
        try
        {
            ...
            Parent root = FXMLLoader.load(getClass().getResource("/com/krupique/bedusystem/interfaces/buscas/BuscaCliente.fxml"));
            paneprincipal.getChildren().clear();
            paneprincipal.getChildren().add(root);

        }catch(Exception er){
            Alert a = new Alert(Alert.AlertType.ERROR, "Erro ao abrir tela de Busca de Clientes! \nErro: " + er.getMessage(), ButtonType.OK);
            a.showAndWait();
        }
    }
